...
- Surge Targets visibility, including sample bars in Preview Frames and Test Mode.
- Independent map and order-frame scales from 50% to 200%. The 100% baseline matches the previous 75% size.
- Shared, uniform frame-background opacity at 0%, 25%, 50%, 75%, or 100%, defaulting to 50% for new settings. Existing saved opacity choices are preserved.
...
Personal Warning Delay controls how long each personal warning stays visible: 1-10 seconds in half-second steps, with a 4-second default. A new warning replaces the previous warning and restarts the display duration.

The dedicated NSRT Macro Buttons section contains the enable toggle, Create NSRT Macros, Attach to Map (on by default), and Auto Show / Hide by Room (off by default). Turn off Attach to Map and Lock Frames to drag the panel by its header; its independent position is saved. Room automation requires the Venomous Abyss instance and the exact English subzone names. Entering `Altar of the Six Winds` opens the panel; entering `The Serpent Warren` or `Pit of Fangs` hides it. It checks zone-change events without polling. Enabling it while in a recognized room applies that room's visibility. Manual visibility changes last until the next room entry. Changes during combat wait until combat ends and follow your latest room. Auto-hide leaves explicit Preview Frames and Test Mode available. Other subzones do not auto-hide the panel; leaving the instance clears it.

Surge Targets is off by default. Enable it under Frame Options, turn on Preview Frames, and turn off Lock Frames to position its two bars. Test Mode's Surge 1/2 buttons animate sample countdowns for the corresponding drop pair. Live targets are inferred from the boss's brief target changes during each configured Surge cast, following NSRT's approach; this is not direct aura confirmation. Each capture displays a ten-second countdown. Completed local assignments take precedence over received NSRT markers; an unknown assignment shows `?`, and drop 4 always uses Exit. Only boss1 target events are monitored, for at most five seconds or two captures per cast. Countdown updates run only while bars are active, at most 20 times per second. Keep the difficulty's Surge cast timestamps accurate and verify target capture in game.

Click the gear beside a difficulty to edit its full fight schedule in chronological order. Each row is one occurrence: Surge 1, Surge 2, Intermission 1, its three wind prompts, rotation stop, intermission end/reset, and subsequent occurrences. All values are time from the pull, displayed as `minutes:seconds` (for example, Heroic Surge 1 at `0:32.22`). You can enter that format or plain seconds. Surge warnings appear 3 seconds before the listed cast time. Wind, rotation, and reset rows occur at the exact listed times. Keep rows in chronological order. Save applies edits on the next pull, Cancel discards the draft, and Reset Defaults fills the draft without saving it.

Normal cast defaults come from [NSRT's encounter alerts](https://github.com/Reloe/NorthernSkyRaidTools/blob/main/NorthernSkyRaidTools/EncounterAlerts/MidnightS2/Sszorak.lua); Heroic and Mythic use [NSRT's boss timelines](https://github.com/Reloe/NorthernSkyRaidTools/blob/main/NorthernSkyRaidTools/BossTimelines/MidnightS2/Sszorak.lua). Wind prompts are initially placed at intermission start, +8, and +18 seconds, rotation stop at +25, and reset at +30, using this addon's existing defaults. Raid Finder defaults to Normal's schedule, with its own independently saved adjustments. Unlisted occurrences on any difficulty use the live timeline and are recorded. Recorded wind and reset times are derived from the intermission timing. Reopen the editor after a pull to see recorded events; Reset Defaults restores published or observed times. The manual Test Mode Intermission button starts immediately with the original 0/8/18-second wind prompts.

The Marker Layout editor keeps the built-in Default profile immutable. Press New to make an editable named copy of the displayed layout. Up to ten profiles, including Default, fit in a two-column grid; custom profiles can be renamed or deleted. An accepted temporary raid layout appears in pink and can be copied into a permanent profile with New. The NSRT macro panel shares the room-map scale and background opacity, including when detached.
...
5. Surge 1 uses drops 1 and 2; Surge 2 uses drops 3 and 4. By default, push prompts occur at Dig In, +8 seconds, and +18 seconds; the difficulty's timing settings can adjust these. All warnings are personal; marker-name TTS requires local selections because secret received values cannot be decoded into names.
6. By default, intermission rotation stops after 25 seconds and local selections/received slots reset 30 seconds after Dig In. The fight schedule can adjust each occurrence independently.
...
Offline regressions also cover room entry/manual hides, deferred attachment changes, saved detached positions, Surge previews, restricted target names and markers, assignment pairs, bounded capture windows, expiry, and encounter cleanup. In game, verify room-entry behavior, drag the detached panel and test its click alignment, and compare both Surge targets and drop icons against the actual debuffs on a live pull.
